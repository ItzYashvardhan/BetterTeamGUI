...
object ConfigManager {
    ...
    fun load(sender: CommandSender) {
        ...
        reloadSafely(JFiles.ALLIES_VIEW.filename, sender) {
            alliesView = loadConfig(JFiles.ALLIES_VIEW); TeamAlliesItem.reload()
        }
        ...
        reloadSafely(JFiles.COLORS.filename, sender) {
            colorsView = loadConfig(JFiles.COLORS); ColorPickerItem.reload()
        }
        reloadSafely(JFiles.DASHBOARD_VIEW.filename, sender) {
            dashBoardView = loadConfig(JFiles.DASHBOARD_VIEW); TeamDashboardItem.reload()
        }
        reloadSafely(JFiles.DIALOG_VIEW.filename, sender) {
            dialogView = loadConfig(JFiles.DIALOG_VIEW); TeamDialogItem.reload()
        }
        reloadSafely(JFiles.FONT.filename, sender) { font = loadConfig(JFiles.FONT) }
        reloadSafely(JFiles.LEADERBOARD_VIEW.filename, sender) {
            leaderBoardView = loadConfig(JFiles.LEADERBOARD_VIEW); LeaderBoardItem.reload()
        }
        reloadSafely(JFiles.LEVELS_VIEW.filename, sender) {
            levelsView = loadConfig(JFiles.LEVELS_VIEW); LevelItem.reload()
        }
        reloadSafely(JFiles.LIST_VIEW.filename, sender) {
            listView = loadConfig(JFiles.LIST_VIEW); TeamListItem.reload()
        }
        reloadSafely(JFiles.INVITE_VIEW.filename, sender) {
            inviteView = loadConfig(JFiles.INVITE_VIEW); InviteViewItem.reload()
        }
        reloadSafely(JFiles.INVITED_PLAYERS_VIEW.filename, sender) {
            invitedPlayersView = loadConfig(JFiles.INVITED_PLAYERS_VIEW); InvitedPlayersListItem.reload()
        }
        reloadSafely(JFiles.MEMBER_MANAGEMENT_VIEW.filename, sender) {
            memberManagementView = loadConfig(JFiles.MEMBER_MANAGEMENT_VIEW); TeamMemberManagementItem.reload()
        }
        reloadSafely(JFiles.MEMBERS_VIEW.filename, sender) {
            membersView = loadConfig(JFiles.MEMBERS_VIEW); TeamMemberItem.reload()
        }
        reloadSafely(JFiles.MONEY_VIEW.filename, sender) {
            moneyView = loadConfig(JFiles.MONEY_VIEW); TeamMoneyItem.reload()
        }
        reloadSafely(JFiles.SETTING_VIEW.filename, sender) {
            settingView = loadConfig(JFiles.SETTING_VIEW); TeamSettingItem.reload()
        }
        reloadSafely(JFiles.TEAM_VIEWER.filename, sender) {
            teamViewer = loadConfig(JFiles.TEAM_VIEWER); TeamViewerItems.reload()
        }
        reloadSafely(JFiles.WARPS_VIEW.filename, sender) {
            warpsView = loadConfig(JFiles.WARPS_VIEW); TeamWarpItem.reload()
        }
        ...
        reloadSafely(JFiles.TEAM_MEMBER_MANAGEMENT_FORM.filename, sender) {
    ...
    fun reloadSafely(name: String, sender: CommandSender, action: () -> Unit) {
        try {
            ...
            ConsoleMessage.printNext(
                "Failed to reload $name: $errorMessage ${ConsoleMessage.Color.RESET}",
                ConsoleMessage.Color.RED
            )
